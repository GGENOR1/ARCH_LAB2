...
protected:
    ...
    std::string get_cached(const std::string &method, const std::string &base_path, const std::string &query, const std::string &basic_auth){
        ...
        std::string result;
        std::cout<<'KEY: '<<key<< ' basic_auth: '<<basic_auth << ' query: '<< query<<' base_path: '<< base_path <<std::endl;
    ...
    std::string get_request(const std::string &method, const std::string &base_path, const std::string &query, const std::string &basic_auth, const std::string &token, const std::string &body)
    {
        try
        {
            ...
            if (!basic_auth.empty())
            {
                ...
                std::cout<<"Authorization Basic " <<std::endl;
            }
            else if (!token.empty())
            {
                request.set("Authorization", "Bearer " + token);
                std::cout<<"Authorization Bearer " <<std::endl;
            }
            std::cout<<"Тут Отправка запроса" <<std::endl;
            session.sendRequest(request);
            std::cout<<"Тут Получение ответа" <<std::endl;
            Poco::Net::HTTPResponse response;
            std::istream &rs = session.receiveResponse(response);
            std::cout<<"Тут Вывод ответа" <<std::endl;
            std::stringstream ss;
